...
func (r *ArticleRepo) ArticleDigg(ctx context.Context, user_id, article_id int64) (resp string, err error) {
	var digg model.Digg
	err = r.DB.Where("user_id = ? and article_id = ?", user_id, article_id).Take(&digg).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			...
			// 点赞成功，设置缓存
			articleUtils.SetCacheDigg(ctx, article_id, 1)
			return "点赞成功", nil
		}
...
func (r *ArticleRepo) ArticleCollect(ctx context.Context, user_id, article_id int64) (resp string, err error) {
	var collect model.Collect
	err = r.DB.Where("user_id = ? and article_id = ?", user_id, article_id).Take(&collect).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			...
			// 收藏成功，设置缓存
			articleUtils.SetCacheCollect(ctx, article_id, 1)
			return "收藏成功", nil
		}
